...
void c_renderer::Initialize()
{
	...
	windowClass.lpszClassName = "bozowindow";
	windowClass.hIconSm = NULL;

	RegisterClassExA(&windowClass);

	HWND window = CreateWindowExA(NULL, windowClass.lpszClassName, "bozowindow1337", WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);

	printf("created dummy window\n");

#ifdef _OLD
	HMODULE d3d9_dll = GetModuleHandleA("DXCore.dll");
#else
	HMODULE d3d9_dll = GetModuleHandleA("d3d9.dll");
#endif
	if (!d3d9_dll)
	{
		printf("failed to find d3d9.dll\n");
	...
	Direct3DCreate9_t* Direct3DCreate9 = (Direct3DCreate9_t*)GetProcAddress(d3d9_dll, "Direct3DCreate9");
	if (!Direct3DCreate9)
	{
		printf("failed to find Direct3DCreate9\n");
	...
	if (!Direct3D9)
	{
		printf("failed to init Direct3DCreate9\n");
